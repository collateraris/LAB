...
void fact(void ) {
	...
	int r = 0, a,b,z,wr,xj,xd,ss;
	char  wrs[0];
	
	if (sum>=sum2) alsum = sum; else alsum = sum2;
	
	for (xi = 1; xi <= alsum;xi++){
		
		n=0;
		for (ss = 3;ss>= 0;ss--){
			
			n++;
			xj = strlen(array[xi].num) - n;
			xd = strlen(array2[xi].num) - n;
			
			
			if ((int)array[xi].num[xj] >= 48) 
			a = (int)array[xi].num[xj] - 48;
			else
			a = 0;
			printf("\nA%d = ",a);
		    if ((int)array2[xi].num[xd] >= 48) 
			b = (int)array2[xi].num[xd] - 48;
			else
			b = 0;
			printf("\nB%d = ",b);
